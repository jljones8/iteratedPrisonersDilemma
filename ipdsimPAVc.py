...
numberOf_PAVc   =  16   # Simple Tester Tit for Tat (defect, then Tit for Tat)
...
if '-s' in argv:
    try:
        ...
        numberOf_PAVc  = int(strategyDistribution[3])
...
N = (numberOf_allC + numberOf_TFT + numberOf_PAVc + numberOf_TFTdcc +
...
def updateDistribution():
    ...
    for n in G:
        ...
        if G.node[n]['abbr'] == 'PAVc'  : count_PAVc   += 1
...
def printDistributionHistory():
    header='Round: ---  allC   allD    TFT   PAVc TFTdcc   GRIM --- Average Score'
    print(header)
    for i in range(r):
        print(
...
def playIPDgame(node_A,node_B):
    ...
    for i in range(iterations):
        ...
        if i == 2:
            ...
            if 'PAVc' in G.node[node_A]['abbr'] and history_A[-1] == 'C' and history_B[-1] == 'C':
                action_A = G.node[node_A]['react_C']
            if 'PAVc' in G.node[node_A]['abbr'] and history_A[-1] == 'C' and history_B[-1] == 'D':
                action_A = G.node[node_A]['switchD']
            if 'PAVc' in G.node[node_A]['abbr'] and history_A[-1] == 'D' and history_B[-1] == 'C':
                action_A = G.node[node_A]['react_D']
            if 'PAVc' in G.node[node_A]['abbr'] and history_A[-1] == 'D' and history_B[-1] == 'D':
                action_A = G.node[node_A]['switchC'] 
            if 'PAVc' in G.node[node_A]['abbr'] and history_A[-1] == 'C' and history_B[-1] == 'C':
                action_A = G.node[node_A]['react_C']
            if 'PAVc' in G.node[node_A]['abbr'] and history_A[-1] == 'C' and history_B[-1] == 'D':
                action_A = G.node[node_A]['switchD']
...
def cullingAndSeeding():
    ...
    for i,(n,score,type) in enumerate(cullList):
        ...
        if seedList[i][2] == 'PAVc'  : agent.update(PAVc)
...
# 08-28-2016 JESS JONES  Changed TFTd to PAVc, BITCHES! 
